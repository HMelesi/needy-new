...
class ExplosionBig extends Explosion {
  ExplosionBig(SpriteSheet sheet) {
    ...
    ParticleSystem particlesFire = new ParticleSystem(
        ...
        colorSequence: new ColorSequence(<Color>[
          new Color(0xffffff33),
          new Color(0xffff3333),
          new Color(0x00ff3333)
        ], <double>[
          0.0,
          0.5,
          1.0
    ...
    Motion scale = new MotionTween<double>((a) {
      spriteRing.scale = a;
    }, 0.2, 1.0, 0.75);
    Motion scaleAndRemove =
        new MotionSequence(<Motion>[scale, new MotionRemoveNode(spriteRing)]);
    Motion fade = new MotionTween<double>((a) {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      Motion scaleAndRemove = new MotionSequence(
          <Motion>[scale, new MotionRemoveNode(spriteFlare)]);
      Motion fadeIn = new MotionTween<double>((a) {
        spriteFlare.opacity = a;
      }, 0.0, 1.0, 0.25 * multiplier);
      Motion fadeOut = new MotionTween<double>((a) {
        spriteFlare.opacity = a;
...
class ExplosionMini extends Explosion {
  ExplosionMini(SpriteSheet sheet) {
    for (int i = 0; i < 2; i++) {
      ...
      MotionTween fade = new MotionTween<double>((a) {
        star.opacity = a;
      }, 1.0, 0.0, 0.2);
      motions.run(fade);
    }

    MotionSequence seq = new MotionSequence(
        <Motion>[new MotionDelay(0.2), new MotionRemoveNode(this)]);
    motions.run(seq);
  }
}
