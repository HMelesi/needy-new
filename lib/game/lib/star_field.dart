...
class StarField extends NodeWithSize {
  ...
  void addStars() {
    ...
    _paddedSize =
        new Size(size.width + _padding * 2.0, size.height + _padding * 2.0);
    ...
    for (int i = 0; i < _numStars; i++) {
      ...
      _colors.add(new Color.fromARGB(
          (255.0 * (randomDouble() * 0.5 + 0.5)).toInt(), 255, 255, 255));
  ...
  void paint(Canvas canvas) {
    ...
    for (int i = 0; i < _numStars; i++) {
      ui.RSTransform transform = new ui.RSTransform(_starScales[i], 0.0,
