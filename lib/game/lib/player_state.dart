...
class PlayerState extends Node {
  PlayerState(this._sheetUI, this._sheetGame, this._gameState) {
    // Score display
    _spriteBackgroundScore = new Sprite(_sheetUI["btn_exit.png"]);
    ...
    // Coin display
    _spriteBackgroundCoins = new Sprite(_sheetUI["btn_play.png"]);
  ...
  void update(double dt) {
    if (_dirtyScore) {
      ...
      for (int i = scoreStr.length - 1; i >= 0; i--) {
        ...
        numSprite.position = new Offset(xPos + i, 0.0);
        addChild(numSprite);
        xPos -= 337.0;
      }
      _dirtyScore = false;
